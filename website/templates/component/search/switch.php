...
// Filtrer les switches
$switchs_data->switchs = array_filter($switchs_data->switchs, function ($s) use ($value) {
    return strpos($s->ip, $value) !== false || strpos(strtolower($s->name), $value) !== false;
});

foreach ($switchs_data->switchs as $s) {

    $warnings = array_filter($warning_data['warnings'], function ($w) use ($s) {
        return $w['switch']['ip'] == $s->ip;
    });
